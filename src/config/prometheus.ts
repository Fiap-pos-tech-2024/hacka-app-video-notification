...
const metricsMiddleware: RequestHandler = (req, res, next) => {
  if (req.path === "/notification-metrics") return next();
  ...
  res.on("finish", () => {
    ...
    totalRequests.inc({ method: req.method, route, status_code: res.statusCode });

    if (res.statusCode >= 400) {
      requestFailures.inc({ method: req.method, route, status_code: res.statusCode });
    }
  });
